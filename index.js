...
function start() {
    ...
    powerUp.startPos = 0;
    powerUp.spaceBetweenRow = 900;
    powerUp.pipeCount = Math.floor(gameArea.offsetWidth / powerUp.spaceBetweenRow);
    for (let i=0; i<powerUp.powerUpCount; i++) {
        makePowerUp(powerUp.startPos * powerUp.spaceBetweenRow);
        powerUp.startPos++;
    }
...
function movePipes(gamer) {
    ...
    pipes.forEach(function(item) { //item에 들어있는 각각의 pipes들
        ...
        if (player.score >= 5000) {
            item.x -= player.speed*2.05;
        }
        if (player.score >= 10000) {
            item.x -= player.speed*2.2;
        }
        if (player.score >= 15000) {
            item.x -= player.speed*2.3;
        }
    ...
    if (player.inPlay == true) {
        ...
        if (keys.Space == true) {
            ...
            if (leftName.innerHTML === 'GROOT') {
                ...
                shootDiv.style.background = 'rgb(99, 78, 34)'
            }
            if (leftName.innerHTML === 'ROCKET') {
